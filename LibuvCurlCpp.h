...
public:
    struct HandleSocketData
    {
        ...
        HandleSocketData()
        {
            ...
            LOG("constructor HandleSocketData " << counter_leak_handlesocketdata)
        }
        ~HandleSocketData()
        {
            counter_leak_handlesocketdata--;
            LOG("destructor HandleSocketData " << counter_leak_handlesocketdata)
        }
    };
    struct CurlContext
    {
        ...
        CurlContext()
        {
            ...
            LOG("constructor CurlContext " << counter_leak_curlcontext)
        }
        ~CurlContext()
        {
            counter_leak_curlcontext--;
            LOG("destructor CurlContext " << counter_leak_curlcontext)
        }
    };
    ...
    struct TimerRequest
    {
        ...
        TimerRequest()
        {
            ...
            LOG("constructor TimerRequest " << counter_leak_timerrequest)
        }
        ~TimerRequest()
        {
            counter_leak_timerrequest--;
            ...
            {
                cout << "~TimerRequest:" << request_headers << endl;

                curl_slist_free_all(request_headers);
                request_headers = NULL;
            }
            LOG("destructor TimerRequest " << counter_leak_timerrequest)
        }
    };
        ...
        while ((message = curl_multi_info_read(curl_handle, &pending)))
        {
            switch (message->msg)
            {
                case CURLMSG_DONE:
                    ...
                    done_cb(std::move(body), std::move(headers), message->data.result, http_code);
                    break;
    ...
    static int startTimeout(CURLM * multi, long timeout_ms, void * userp)
    {
        ...
        cout << "request_headers:" << timer_req->request_headers << endl;
        if (timeout_ms < 0)
        {
            ...
            case CURL_POLL_REMOVE:
                ...
                if (socketp)
                {
                    ...
                    uv_close(reinterpret_cast<uv_handle_t *>(&curl_context->poll_handle), [](uv_handle_t * handle) {
                        delete reinterpret_cast<CurlContext *>(handle->data);
                    });
